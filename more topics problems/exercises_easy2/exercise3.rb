...
# def map(arr)
#   result = []
#   arr.each { |e| result << yield(e) }
#   result
# end

def map(arr)
  arr.inject([]) { |result, element| result << yield(element) }
end
...
=begin --> Rodney M. solution for FE, works with other types

def map(col)
  col.each_with_object(col.class.new) { |el, result| result << yield(el) }
end
